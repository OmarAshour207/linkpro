            ...
            <div class="card card-form__body card-body">
                <form method="post" action="{{ route('offices.update', $office->id) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input id="title" name="title" dir="auto" type="text" class="form-control" placeholder="{{ __("Title") }}" value="{{ old("title", $office->title) }}">
                    </div>
                    ...
                    <div class="form-group col-lg-6">
                        ...
                        <select id="company" name="user_id" data-toggle="select" class="form-control select2 company_id">
                            ...
                            @forelse($companies as $company)
                                <option value="{{ $company->id }}" {{ old('user_id', $office->user_id) == $company->id ? 'selected' : '' }}>
                                    {{ $company->name }}
                                </option>
                            @empty
                            @endforelse
                    ...
                    <div class="form-group col-lg-6">
                        ...
                        <select id="floor_id" name="floor_id" data-toggle="select" class="form-control select2 floors">
                            ...
                            @forelse($floors as $floor)
                                <option value="{{ $floor->id }}" {{ old('floor_id', $office->floor_id) == $floor->id ? 'selected' : '' }}>
                                    {{ $floor->title }}
                                </option>
                            @empty
                            @endforelse
                    ...
                    <div class="form-group col-lg-6">
                        ...
                        <select id="path_id" name="path_id" data-toggle="select" class="form-control select2 paths">
                            ...
                            @forelse($paths as $path)
                                <option value="{{ $path->id }}" {{ old('path_id', $office->path_id) == $path->id ? 'selected' : '' }}>
                                    {{ $path->title }}
                                </option>
                            @empty
                            @endforelse
